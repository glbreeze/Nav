...
try:
    ...
    # For older PyTorch where torch.nn.attention is unavailable.
    from torch.backends.cuda import SDPBackend, sdp_kernel as sdpa_kernel
    _NEW_SDPA_API = False
...
def _sdpa_ctx(use_flash: bool):
    """Compatibility wrapper for torch SDPA kernel context across versions."""
    if _NEW_SDPA_API:
        if use_flash:
...
class FlashAttention(Attention):
    def forward(self, x: Tensor, attn_bias=None) -> Tensor:
        ...
        with _sdpa_ctx(q.dtype == torch.bfloat16):
...
class PRopeFlashAttention(AttentionRope):
    def forward(self, x: Tensor, extrinsics, H, W, patch_h, patch_w, K=None, attn_mask=None) -> Tensor:
        ...
        with _sdpa_ctx(q.dtype == torch.bfloat16 and attn_mask is None):
